...
class StrokeEngine:
    ...
    def process(self, fractals: list[Fractal], merged_bars: list[MergedBar], raw_bar_count: int) -> tuple[list[Stroke], list[LifecycleEvent]]:
        if len(fractals) < 2:
            return [], []
        strokes: list[Stroke] = []
        ...
        for candidate in fractals[1:]:
            if candidate.fractal_type == anchor.fractal_type:
                ...
                continue
            valid, reason, detail = self._validate(anchor, candidate, merged_bars)
            if not valid:
                events.append(LifecycleEvent(
                    event_type=EventType.CANDIDATE_REJECTED, object_type="stroke_candidate",
                    ...
                    reason_code=reason, rule_profile=self.rule_profile,
                    rule_version=self.rule_version, detail=detail,
                ))
                continue
            stroke = self._create_stroke(anchor, candidate, merged_bars, counter)
            counter += 1
            events.append(LifecycleEvent(
                event_type=EventType.CREATED, object_type="stroke", object_id=stroke.object_id,
                logical_id=stroke.logical_id, occurred_at_bar_id=candidate.right_bar_id or candidate.merged_bar_id,
                reason_code="STRICT_STROKE_VALID", rule_profile=self.rule_profile,
                rule_version=self.rule_version,
                detail={"start_bar_index": stroke.start_bar_index, "end_bar_index": stroke.end_bar_index,
            ...
            if strokes:
                ...
                if previous.end_fractal_id == stroke.start_fractal_id and previous.status == StructureStatus.PROVISIONAL:
                    ...
                    events.append(LifecycleEvent(
                        event_type=EventType.CONFIRMED, object_type="stroke", object_id=previous.object_id,
                        logical_id=previous.logical_id,
                        occurred_at_bar_id=candidate.right_bar_id or candidate.merged_bar_id,
                        reason_code="NEXT_STRICT_STROKE_CONFIRMED", rule_profile=self.rule_profile,
                        rule_version=self.rule_version,
                    ))
    ...
    def _validate(self, f1: Fractal, f2: Fractal, bars: list[MergedBar]) -> tuple[bool, str, dict]:
        ...
        if start < 0 or end >= len(bars) or end <= start:
            return False, "INVALID_BAR_RANGE", {"start": start, "end": end}
    ...
    def _create_stroke(self, f1: Fractal, f2: Fractal, bars: list[MergedBar], counter: int) -> Stroke:
        direction = StrokeDirection.UP if f1.fractal_type == FractalType.BOTTOM else StrokeDirection.DOWN
        interval = bars[f1.merged_bar_index:f2.merged_bar_index + 1]
        stroke = Stroke(
            stroke_id=f"stroke_{counter:06d}", direction=direction,
            start_fractal_id=f1.fractal_id, end_fractal_id=f2.fractal_id,
            start_price=f1.price, end_price=f2.price,
            start_bar_index=f1.merged_bar_index, end_bar_index=f2.merged_bar_index,
            merged_bar_count=len(interval), max_price=max(b.high for b in interval),
            min_price=min(b.low for b in interval), price_range=abs(f2.price - f1.price),
            object_id=f"stroke_{counter:06d}_r1",
            logical_id=f"stroke:{f1.logical_id}->{f2.logical_id}", revision=1,
            status=StructureStatus.PROVISIONAL, created_at_bar=f2.window_indices[-1],
            repaint_risk="HIGH", confirmation_requirements=["next strict stroke must confirm"],
